...
const isMongoose5 = semver.gte(mongoose.version, '5.0.0')
...
var mongooseEncryption = function(schema, options) {
    ...
    if (options.middleware) { // defaults to true
        schema.pre('init', function(next, data) {
            if (isMongoose5) {
                // 5.x does not provide a 'next' function for this middleware since it's synchronous
                data = next
            }
            var err = null;
        ...
        if (options.decryptPostSave) { // true by default
            schema.post('save', function(doc) {
                if (isMongoose5 && isEmbeddedDocument(doc)) {
                    return doc;
                }
