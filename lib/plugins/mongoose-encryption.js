...
const mongooseEncryption = function mongooseEncryption(
  ...
  if (secret) {
    if (encryptionKey || signingKey) {
      throw new Error(
        "if secret is used, then encryptionKey and signingKey must not be included"
      );
    } else {
    ...
    throw new Error(
      "must provide either secret or both encryptionKey and signingKey"
    );
  } else {
  ...
  if (deprecatedFieldsOption) {
    ...
    console.warn(
      "the 'fields' option has been deprecated. please use 'encryptedFields'"
    );
  }
  if (deprecatedExcludeOption) {
    excludeFromEncryption = deprecatedExcludeOption;
    console.warn(
      "the 'exclude' option has been deprecated. please use 'excludeFromEncryption'"
    );
  }
  ...
  function computeAC(doc, fields, version, modelName) {
    ...
    const documentCollectionId =
      collectionId || modelName || doc.constructor.modelName;
    ...
    const objectToAuthenticate = pick(
  ...
  if (middleware) {
    ...
    schema.pre("save", async function preSave() {
      if (this.isNew || this.isSelected("_ct")) {
        await this.encrypt();
        if (
          (this.isNew || allAuthenticationFieldsSelected(this)) &&
  ...
  schema.methods.sign = async function sign() {
    ...
    const authenticatedFieldsBuf = Buffer.from(
