...
var conflictUtil = require('../util/conflict-util.js');
...
var getExtraData = conflictUtil.getExtraData;
...
var handleDecryptionConflictDefault = function (diff) {
    var doc = diff.doc
    
    var error = new Error(
        'Conflict detected when decrypting ' + doc.constructor.modelName + ' record.\n\n' +
        (doc.id ? 'Record ID: ' + doc.id : '') +
        'Use handleDecryptionConflict option to resolve data conflicts.'
    );

    /**
     * To get the diff data a function should be called explicitly,
     * to prevent exposing potentially secret data in logs.
     */
    error.getDiff = function () {
        return diff;
    }
...
var mongooseEncryption = function(schema, options) {
    var encryptedFields, excludedFields, authenticatedFields, encryptionKey, signingKey, handleDecryptionConflict;
    ...
    /** Decryption Options */
    if (options.handleDecryptionConflict !== undefined) {
        handleDecryptionConflict = options.handleDecryptionConflict
    } else {
        handleDecryptionConflict =  handleDecryptionConflictDefault;
    }
    ...
    schema.methods.decryptSync = function(doc) {
        var that = this;
        var ct, ctWithIV, decipher, iv, idString, decryptedObject, decryptedObjectJSON, decipheredVal, unSecretData, unExposedData;
        if (this._ct) {
            ...
            if (handleDecryptionConflict && doc && _.isFunction(doc.$isDefault)) {
                // Default values can be assigned to secret fields upon initialization.
                // Because of that, we can only check un-secret conflicts when $isDefault is available,
                // otherwise we cannot tell if a secret field has been initialized with default value.
                unExposedData = pick(
                    this,
                    _.filter(encryptedFields, field => !doc.$isDefault(field)),
            ...
            if (handleDecryptionConflict && doc) {
                unSecretData = getExtraData(encryptedFields, decryptedObject);
            }
            ...
            if (handleDecryptionConflict && doc && (!_.isEmpty(unSecretData) || !_.isEmpty(unExposedData))) {
                handleDecryptionConflict({doc, encryptedFields, unSecretData, unExposedData});
            }
